...
export const PostInvoiceFromDeployment = async (InvoiseInfo:any,AdvanceAmount:any,InvoiceNumber:any,ImpDate:any,ImpLastDate:any) => {
  try {
...
PreferdWorkingStates: Info.PreferdWorkingStates || "",
   ...
   Documents: {
  ProfilePic: Info.Documents?.ProfilePic || null,
  AadharCard: Info.Documents?.AadharCard || null,
  PanCard: Info.Documents?.PanCard || null,
  AccountPassBook: Info.Documents?.AccountPassBook || null,
  CertificateOne: Info.Documents?.CertificatOne || null,
  CertificateTwo: Info.Documents?.CertificatTwo || null,
  BVR: Info.Documents?.BVR || null,
  HCPForm: Info.Documents?.HCPForm || null,
  HealthCertificate: Info.Documents?.HealthCertificate || null,
  ReferenceCertificate: Info.Documents?.ReferenceCertificate || null,
  Other: Info.Documents?.Other || null,
},
...
export const InserTerminationData=async(ClientUserId:any,HCAUserId:any,HCA_Name:any,Name:any,Email:any,Contact:any,ClientAdress:any,Contacthca:any,TimeSheetArray:any,ClientAttendece:any,TerminatedTimeSheet:any,ImpState:any)=>{
  try{
const cluster=await clientPromise
const db=cluster.db("CurateInformation")
...
export const UpdatePaymentVerificationStatusInDb = async (
  HCAId: string,
  ClientId: string,
  value: string,
  MonthInfo: string,
...
export const UpdatePaymentVerificationStatusInReplacementDb = async (
  HCAId: string,
  ClientId: string,
  value: string,
  MonthInfo: string
...
export const PostINRejectionDb = async (
  ...
  try {
    if (!RejectionInfo || !Reason || !MonthInfo) {
      return {
        success: false,
        message: "Missing required parameters",
      };
...
const deleteResult = await payableCollection.deleteMany(filter);
    return {
      success: true,
      insertedId: result.insertedId.toString(),
      message: "Rejection information saved successfully",
    };
  } catch (error: any) {
    console.error("PostINRejectionDb Error:", error);

    return {
      success: false,
      message: error?.message || "Failed to save rejection information",
    };
  }
};
export const PostINSuccesfulPaymentsDb = async (
  PaymentInfo:any,
 ...
 if (
  ...
  !String(MonthInfo || '').trim() ||
  PaymentInfo.ClientId == null ||
  PaymentInfo.HCAId == null
) {
      return {
        success: false,
        message: "Missing required parameters",
    ...
    const successfulPaymentsCollection =
...
const HCPPaymentInfo = {
 ...
 const deleteResult = await payableCollection.deleteMany(filter);
    await payableCollection.updateOne(
      ...
      {
        $set: {
          NeftNumber: String(NeftTransactionNumber).trim(),
          UpdatedAt: now,
        },
      }
    );
    ...
    return {
      success: true,
      insertedId: insertResult.insertedId.toString(),
      message: "Successful payment information saved successfully",
    };
  } catch (error: any) {
    console.error("PostINSuccesfulPaymentsDb Error:", error);

    return {
      success: false,
      message:
        error?.message || "Failed to save successful payment information",
    };
  }
};
...
export const UpdateStatusEnableinRepleasment = async (
  HCAId: string,
  ClientId: string,
  MonthInfo: string
) => {
...
export const updateExpense = async (
  ...
  try {
    ...
    const cluster = await clientPromise;
    const db = cluster.db("CurateInformation");
    const completeCollection = db.collection(
      "CompliteRegistrationInformation"
    );

   

    const existingMonth = await completeCollection.findOne({
      "HCAComplitInformation.UserId": userId,
      "HCAComplitInformation.MonthlyExpenses.Month": currentMonth,
    });

    let result;

   if (existingMonth) {
  result = await completeCollection.updateOne(
    {
      "HCAComplitInformation.UserId": userId,
      "HCAComplitInformation.MonthlyExpenses.Month": currentMonth,
    },
    {
      $set: {
        "HCAComplitInformation.MonthlyExpenses.$.DueAmounts": dueAmount,
        "HCAComplitInformation.MonthlyExpenses.$.LastDueAmountsUpdatedBy":
          importedEmail,
        "HCAComplitInformation.MonthlyExpenses.$.LastDueAmountsUpdatedAt":
          new Date(),
      },
      $push: {
        "HCAComplitInformation.MonthlyExpenses.$.Transactions": {
          $each: Transactions,
        } as any,
      },
    }
        ...
        },
        {
          $push: {
            "HCAComplitInformation.MonthlyExpenses": {
              Month: currentMonth,
              DueAmounts: [dueAmount] as any,
              Transactions,
              LastDueAmountsUpdatedBy: importedEmail,
              LastDueAmountsUpdatedAt: new Date(),
            } as any,
          },
          $setOnInsert: {
            "HCAComplitInformation.UserId": userId,
