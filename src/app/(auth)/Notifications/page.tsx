...
import { Refresh, UpdateCallEnquiryInformation } from "@/Redux/action";
...
export default function NotificationsCenter() {
  ...
  const handleAction = async (
    ...
    const sendWhatsApp = async (Impmessage: string) => {
  ...
  const UpdateCallEnquiryInfo = async (CallEnquiryInformation: any) => {
    
    try {
         const cleanedData = {
      ...CallEnquiryInformation,
      createdAt: CallEnquiryInformation.createdAt
        ? new Date(CallEnquiryInformation.createdAt).toISOString()
        : null,
    };
      dispatch(UpdateCallEnquiryInformation(cleanedData));
      router.push("/DashBoard")

    } catch (error) {
      console.error("Error updating call enquiry information:", error);
    }
