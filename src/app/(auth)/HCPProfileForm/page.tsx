...
import { GetRegidterdUsers, GetUserInformation, GetHCACompleteInformation, HCARegistration, PostHCAFullRegistration, RegisterHCAWithProfile, UpdateHCARegistration, UpdateHCAFullRegistration, UpdateFinelVerification } from '@/Lib/user.action';
...
import { EyeOff, Eye, CheckCircle, XCircle, LogOut, Edit3, Save, ArrowLeft, TrendingUp, Landmark, History as HistoryIcon } from 'lucide-react';
import { getDaysInMonth } from '@/Lib/Actions';
import HCPReviews from '@/Components/Reviews/page';
import FullYearTransactions from '@/Components/FullYearHistory/page';
import HikeHistory from '@/Components/HikeHistory/page';
import PaymentPassbook from '@/Components/HCAStatastics/page';
...
const documentLabels: Record<string, string> = {
  ...
  const [isPreviewMode, setIsPreviewMode] = useState(Boolean(previewUserId));
...
console.log("Check Docs-----",Docs)
  const [form, setForm] = useState<FormState>({
    ...
    Password: '',
    ConfirmPassword: '',
    PreviewUserType:CurrentUserType || (previewUserId ? "HCA" : "HCA"),
    referralSourceType:'',
    Guardian:'',
    GuardianContact:'',
    PermanentState:'',
  ...
  // Existing HCA selected from the HCA list for Preview/Edit.
  // Normal registration has no previewUserId and keeps the original flow.
...
// }, [RegisterfromAdmin, previewUserId]);
  ...
  // Load the selected existing HCA and populate the SAME registration form.
  useEffect(() => {
    if (!previewUserId) return;

    let cancelled = false;

    const loadExistingHCA = async () => {
      try {
        setIsChecking(true);
        setUpdatedStatusMessage('Loading HCA information...');
        ...
        setisuserIdAvailable(previewUserId);
        ...
        setForm((prev: any) => ({
          ...
          fatherNameContact: info.fatherNameContact ?? '',
          motherName: info.motherName ?? '',
          motherContact: info.motherContact ?? '',
          ...
          aadharCardNo: info.aadharCardNo ?? '',
          panNumber: info.panNumber ?? '',
          PermanentHouseNo: info.PermanentHouseNo ?? '',
          ...
          cityPostcodePermanent: info.cityPostcodePermanent ?? '',
          cityPostcodeCurrent: info.cityPostcodeCurrent ?? '',
          higherEducation: info.higherEducation ?? '',
          higherEducationYearStart: info.higherEducationYearStart ?? '',
          higherEducationYearEnd: info.higherEducationYearEnd ?? '',
          professionalEducation: info.professionalEducation ?? '',
          professionalEducationYearStart: info.professionalEducationYearStart ?? '',
          professionalEducationYearEnd: info.professionalEducationYearEnd ?? '',
          registrationCouncil: info.registrationCouncil ?? '',
          registrationNo: info.registrationNo ?? '',
          professionalSkill: Array.isArray(info.professionalSkill) ? info.professionalSkill : [],
          HandledSkills: Array.isArray(info.HandledSkills) ? info.HandledSkills : [],
          HomeAssistance: Array.isArray(info.HomeAssistance) ? info.HomeAssistance : [],
          certifiedBy: info.certifiedBy ?? '',
          professionalWork1: info.professionalWork1 ?? '',
          professionalWork2: info.professionalWork2 ?? '',
          experience: info.experience ?? '',
          height: info.height ?? '',
          weight: info.weight ?? '',
          hairColour: info.hairColour ?? '',
          eyeColour: info.eyeColour ?? '',
          complexion: info.complexion ?? '',
          anyDeformity: info.anyDeformity ?? '',
          moleBodyMark1: info.moleBodyMark1 ?? '',
          moleBodyMark2: info.moleBodyMark2 ?? '',
          SiblingsInfo: Array.isArray(info.SiblingsInfo) ? info.SiblingsInfo : prev.SiblingsInfo,
          OngoingStudy: info.OngoingStudy ?? '',
          reportPreviousHealthProblems: info.reportPreviousHealthProblems ?? '',
          reportCurrentHealthProblems: info.reportCurrentHealthProblems ?? '',
          sourceOfReferral: info.sourceOfReferral ?? '',
          dateOfReferral: info.dateOfReferral ?? '',
          reference1Name: info.reference1Name ?? '',
          reference1Aadhar: info.reference1Aadhar ?? '',
          reference1Mobile: info.reference1Mobile ?? '',
          reference1Address: info.reference1Address ?? '',
          reference1Relationship: info.reference1Relationship ?? '',
          reference2Name: info.reference2Name ?? '',
          reference2Aadhar: info.reference2Aadhar ?? '',
          reference2Mobile: info.reference2Mobile ?? '',
          reference2Address: info.reference2Address ?? '',
          serviceHours12hrs: Boolean(info.serviceHours12hrs),
          serviceHours24hrs: Boolean(info.serviceHours24hrs),
          preferredService: info.preferredService ?? '',
          ...
          paymentBankAccountNumber: info.paymentBankAccountNumber ?? '',
          ifscCode: info.ifscCode ?? '',
          bankBranchAddress: info.bankBranchAddress ?? '',
          Bankbranchname: info.Bankbranchname ?? '',
          Branchcity: info.Branchcity ?? '',
          Branchstate: info.Branchstate ?? '',
          Branchpincode: info.Branchpincode ?? '',
          ...
          preferredWorkStates: Array.isArray(info.preferredWorkStates) ? info.preferredWorkStates : [],
          Password: BasicInfo?.PreviewPassword || '',
          Reviews:info.Reviews||[],
          PaymentHistory:info.PaymentHistory||[],
          HikeHistory:info.HikeHistory||[],
        }));

        setSiblings(Array.isArray(info.SiblingsInfo) ? info.SiblingsInfo : [
          { relation: 'Elder Brother', count: 0 },
          { relation: 'Younger Brother', count: 0 },
          { relation: 'Elder Sister', count: 0 },
          { relation: 'Younger Sister', count: 0 },
        ...
        console.error('Error loading HCA for preview:', error);
        setUpdatedStatusMessage('Unable to load HCA information.');
      } finally {
...
if (!form.PaymentforStaff?.trim() || payment <= 0) {
  ...
  return;
      ...
      if (CurrentUserType === null && !previewUserId) {
...
if (!isAnyFieldEmpty && !isReasonEmpty) {
  ...
  return;
          ...
          const generatedUserId = previewUserId || userId || localStorage.getItem('UserId') || uuidv4();

          const localValue =
            previewUserId || userId || localStorage.getItem("UserId");
          ...
          const FinelForm = {
            ...
            setUpdatedStatusMessage(
              isExistingHCAMode
                ? "Updating HCA information..."
                : "Creating account and saving profile..."
            );

            let Complitinfo: any;
            let PrimaryDetails: any;

            if (isExistingHCAMode) {
              Complitinfo = await UpdateHCAFullRegistration(generatedUserId, FinelForm);
              if (!Complitinfo?.success) {
                setUpdatedStatusMessage(Complitinfo?.message || "Unable to update complete HCA information.");
                return;
              }

              PrimaryDetails = await UpdateHCARegistration(generatedUserId, payload);
              if (!PrimaryDetails?.success) {
                setUpdatedStatusMessage(PrimaryDetails?.message || "Unable to update HCA account information.");
                return;
              }
            } else {
              Complitinfo = await PostHCAFullRegistration(FinelForm);
              if (!Complitinfo?.success) {
                setUpdatedStatusMessage(Complitinfo?.message || "Registration failed.");
                return;
              }

              PrimaryDetails = await HCARegistration(payload);
              if (!PrimaryDetails?.success) {
                setUpdatedStatusMessage(PrimaryDetails?.message || "Registration failed.");
                return;
              }
            ...
            setUpdatedStatusMessage(
              isExistingHCAMode
                ? "HCA information updated successfully."
...
const PaymentInfoPreview = () => {
  switch (CurrentPaymentView) {
    case "MonthlySalarySlip":
      return (
        <PaymentPassbook
          PAYMENT_HISTORY={form.PaymentHistory}
          HCAName={`${form.firstName} ${form.surname}`}
        />
      );
...
if (isChecking || (CurrentUserType === undefined && !previewUserId)) {
  return <LoadingData />;
}
        ...
        <div className="bg-white border border-gray-200 rounded-3xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-500">
          ...
          <div className="flex md:h-[170px] flex-col md:flex-row items-center md:items-stretch">
            ...
            <div className="flex-1 p-4  text-center md:text-left flex flex-col justify-center">
              ...
              {isPreviewMode && previewUserId ? 'Health Care Assistant' : FullForms(EffectiveUserType) + ' Registration'}
              </h1>
    ...
    <button
            ...
            >
              ...
              {UpdatedStatusMessage && (
                <div
                  className={`w-full max-w-md p-4 rounded-xl font-medium text-sm shadow-inner transition-all duration-300 ${UpdatedStatusMessage === "Successfully Updated Your Information."
      ...
      <div className="w-full px-4 md:px-8 pt-4">
        <div className="max-w-[1600px] mx-auto flex flex-col sm:flex-row items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white/95 backdrop-blur px-4 py-3 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-slate-800">
                {isPreviewMode ? 'Preview HCA Details' : (previewUserId ? 'Edit HCA Details' : 'HCA Registration')}
              </p>
              {previewUserId && (
...
<div className="flex items-center gap-3">
  ...
  <button
    ...
    className={`cursor-pointer rounded-xl border px-5 py-2.5 text-sm font-semibold transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 ${
      ShowReviews
        ? "border-indigo-600 bg-gradient-to-r from-indigo-600 to-violet-600 text-white shadow-md shadow-indigo-500/25"
        : "border-slate-200 bg-white text-slate-700 shadow-sm hover:-translate-y-0.5 hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-600 hover:shadow-md"
    }`}
  >
    Reviews
  </button>
  ...
  >
    Payments
          ...
          {previewUserId && (
            <button
              ...
              className={isPreviewMode
                ? "inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-teal-600 to-cyan-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg hover:shadow-xl transition"
                : "inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition"}
            >
              {isPreviewMode ? <Edit3 size={16} /> : <Save size={16} />}
              {isPreviewMode ? 'Edit Details' : 'Preview Mode'}
            </button>
          )}
      ...
      >
            {ShowReviews && previewUserId && (
          <HCPReviews  HCAName={`${form.firstName} ${form.surname}`} UserId={previewUserId} ImportedReviews={form.Reviews||[]}/>
      )}

{Payments && previewUserId && (
  <>
    <div className="mb-6 rounded-2xl border border-slate-200/80 bg-slate-50/70 p-2 shadow-sm">
      <div className="flex flex-wrap gap-2">
        {/* Monthly Salary Slip */}
        <button
          onClick={() => setCurrentPaymentView("MonthlySalarySlip")}
          className={`group relative flex min-h-[48px] flex-1 items-center justify-center gap-2.5 rounded-xl px-5 py-3 text-sm font-semibold transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-[#1392d3]/30 ${
            CurrentPaymentView === "MonthlySalarySlip"
              ? "bg-gradient-to-r from-[#1392d3] to-[#087fbd] text-white shadow-lg shadow-[#1392d3]/25"
              : "bg-white text-slate-600 shadow-sm hover:-translate-y-0.5 hover:bg-[#1392d3]/5 hover:text-[#1392d3] hover:shadow-md"
          }`}
        >
          <Landmark
        ...
        <button
          ...
          className={`group relative flex min-h-[48px] flex-1 items-center justify-center gap-2.5 rounded-xl px-5 py-3 text-sm font-semibold transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-[#50c896]/30 ${
            CurrentPaymentView === "FullYearTransactions"
              ? "bg-gradient-to-r from-[#50c896] to-[#36ad7b] text-white shadow-lg shadow-[#50c896]/25"
              : "bg-white text-slate-600 shadow-sm hover:-translate-y-0.5 hover:bg-[#50c896]/5 hover:text-[#36ad7b] hover:shadow-md"
          }`}
        >
          <HistoryIcon
            className={`h-5 w-5 transition-transform duration-300 group-hover:scale-110 ${
        ...
        <button
          ...
          className={`group relative flex min-h-[48px] flex-1 items-center justify-center gap-2.5 rounded-xl px-5 py-3 text-sm font-semibold transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-[#ff1493]/30 ${
            CurrentPaymentView === "HikeHistory"
              ? "bg-gradient-to-r from-[#ff1493] to-[#e00078] text-white shadow-lg shadow-[#ff1493]/25"
              : "bg-white text-slate-600 shadow-sm hover:-translate-y-0.5 hover:bg-[#ff1493]/5 hover:text-[#ff1493] hover:shadow-md"
          }`}
        >
          <TrendingUp
            className={`h-5 w-5 transition-transform duration-300 group-hover:scale-110 ${
    ...
    {PaymentInfoPreview()}
        ...
        <fieldset disabled={isPreviewMode} className="min-w-0 border-0 p-0 m-0">
...
>
  ...
  />
            ...
            {((EffectiveUserType === "HCA") || (EffectiveUserType === "HCN")) && (
              <div>
...
{/* <div className="flex justify-end">
  ...
  >
    ...
    Save
            ...
            {EffectiveUserType === "HCA" ? <div className="space-y-5">
      ...
      +{each} ft
                  ...
                  {form.height && (
                    <p className="mt-3 bg-pink-400 p-2 text-white rounded-md text-xs sm:text-sm text-center">
                      {form.height}ft {EffectiveUserType} Height
                    </p>
                  )}
                </div>
                ...
                {form.weight && (
                  <div className="flex justify-center">
                    <p className="mt-3 w-[200px] text-center bg-pink-400 p-2 text-white rounded-md text-xs sm:text-sm">
                      {form.weight} {EffectiveUserType} Weight (kg)
                    </p>
                  </div>
                )}
              </div>
...
>
  ...
  {docKey === "CertificatOne"
    ? "Education"
    : docKey === "CertificatTwo"
    ? "Experiance"
        ...
        <div className="flex justify-center mt-8">
          ...
          <button
            ...
            className={`px-10 py-3 rounded-full text-white font-semibold shadow-lg transition-all duration-300
              ...
              `}
          >
            {UpdateingStatus ? (isExistingHCAMode ? 'Save HCA Changes' : 'Register Profile') : (isExistingHCAMode ? 'Saving...' : 'Registering...')}
          </button>
        </div>
