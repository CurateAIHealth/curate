...
export default function HCPHome() {
  ...
  const hostelApplicable = !workStatus.placement && !workStatus.training;
  ...
  return (
    <>
        ...
        <div className="w-full px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button className="relative">
              ...
              <span className="absolute -top-1 -right-1 w-2 h-2 bg-[#ff1493] rounded-full" />
            </button>
      ...
      <main className="w-full px-4 py-6 space-y-6">
        ...
        <div className="bg-white rounded-2xl p-5 shadow border border-gray-100 flex flex-col sm:flex-row sm:justify-between gap-4">
          <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <ActionButton
            label="Show Hostel Information"
            active={showHostelInfo}
            onClick={() => {
        ...
        {showNetPay && (
          <div className="bg-white rounded-2xl p-5 shadow border border-gray-100">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="border-t pt-3 mt-3 flex justify-between text-base font-extrabold">
                <span>Net Pay</span>
...
function InfoRow({ label, value, highlight }: any) {
  return (
    <div className="flex justify-between">
      ...
      <span className={`font-medium ${highlight ? "text-[#ff1493]" : "text-gray-800"}`}>
        {value}
      </span>
    </div>
  );
...
function ActionButton({ label, onClick, active }: any) {
  return (
    <button
      onClick={onClick}
      className={`
        rounded-xl p-4 text-sm font-semibold border transition
        ${
          active
